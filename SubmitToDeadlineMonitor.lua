...
function SanityCheck(comp)
	...
	--local savers = bmd.GetSavers(comp)
	--local loaders = bmd.GetLoaders(comp)
...
-- This function gathers "sticky" settings and saves them to a JSON file.
--
-- These settings are not set/handled anywhere else in this version of the integrated submitter for Fusion. They are
-- used in the version for prior versions of Fusion (SubmitToDeadline.eyeonscript in this directory) where the UI uses
-- IUP (Fusion's UI framework). Some customers have custom sanity check scripts
--
-- (see 
-- https://docs.thinkboxsoftware.com/products/deadline/10.0/1_User%20Manual/manual/app-fusion.html#custom-sanity-check-setup
-- )
--
-- that overwrite them prior to opening the integrated submitter to guide artists to the recommended submission settings
-- for the studio. To maintain backwards-compatibility with custom sanity checks that do this, we collect the settings
-- and pass them to the Monitor submitter which prioritizes them over the local machine's sticky settings.
function writeDeadlineSettings()
	
	settingNames = {
		"DEADLINE_AutoTimeout",
		"DEADLINE_CheckOutput",
		"DEADLINE_DefaultChunkSize",
		"DEADLINE_DefaultCommandLineMode",
		"DEADLINE_DefaultDepartment",
		"DEADLINE_DefaultFirstAndLast",
		"DEADLINE_DefaultGroup",
		"DEADLINE_DefaultMachineLimit",
		"DEADLINE_DefaultPool",
		"DEADLINE_DefaultPriority",
		"DEADLINE_DefaultSecondaryPool",
		"DEADLINE_DefaultSlaveTimeout",
		"DEADLINE_LimitGroups",
		"DEADLINE_SubmitComp"
	}
	settings = {}
	hasElements = false
...
-- Save any "sticky" settings that may have been written by a custom sanity check script to a JSON file
